...
RemoteServer              = 'eos.cern.ch'
RemoteStorageDir          = '/eos/user/l/leyan/HSCP2024/HSCP_AN17_192/CMSSW_8_0_30/SUSYBSMAnalysis/HSCP/test/AnalysisCode/Results'
        ...
        for line in f :
           ...
           if((vals[0].replace('"','')) in CMSSW_VERSION):
              for Type in AnalysesToRun:
                 ...
                 if vals[2].find("13TeV16") < 0: continue # skip everything that is not 2016 -- namely the 2015 samples, and 2016 postG samples, as they are checked processed automatically with 2016 pre-G and combined samples
...
#              if(vals[2].find("8TeV")<0):continue
        ...
        for line in f :
           ...
           if vals[2].find("13TeV16G") > 0: continue # only run on the 13TeV16 pre-G for combine, post-G will be automatically processed with it
           for Type in AnalysesToRun:            
              if(int(vals[1])>=2 and skipSamples(Type, vals[2])==True):continue     
              skip = False
